# -*- encoding: utf-8 -*-
'''
@File    :   evaluate_mnt_torch.py
@Time    :   2024/09/02 16:18:44
@Author  :   panzhiyu 
@Version :   1.0
...
from models.dataloader_densemnt import VirtDataset, MatchDataset
...
def pad_collate_fn(batch):
    def pad_to_max_N(tensor_list, target_N):
        ...
        for tensor in tensor_list:
            ...
            if current_N < target_N:
                # 使用NaN填充
                padding_size = (0, 0, 0, target_N - current_N)  # 只在N维度进行补齐
                padded_tensor = torch.nn.functional.pad(tensor, padding_size, value=float('nan'))
            else:
                padded_tensor = tensor
            padded_list.append(padded_tensor)
        return torch.stack(padded_list, dim=0)  # 将补NaN后的张量堆叠成一个 batch, BxNxC
    ...
    search_mnt = [torch.tensor(item["search_mnt"], dtype=torch.float32) for item in batch]  # 将numpy转换为tensor
    ...
    batch_dict = {
        ...
        "index": torch.stack(index)  # 如果 index 也是张量，则直接堆叠
    }

    return batch_dict #default_collate(batch)
...
def calculate_score_torchB(feat1, feat2, mask1, mask2, ndim_feat=6, N_mean=1327, Normalize=False, binary=False, f2f_type=(2, 1)):
    ...
    '''
    feat1_dense = feat1
    feat1_mask = mask1.repeat(1, 1, ndim_feat) # unsqueeze(-1).

    feat2_dense = feat2
    feat2_mask = mask2.repeat(1, 1, ndim_feat) # unsqueeze(-1).
...
def lsar_score_torchB(S, mnt1, mnt2, min_pair=4, max_pair=12, mu_p=20, tau_p=0.4):
    ...
    def relax_labeling(mnts1, mnts2, scores, min_number, n_pair): # min_number is the valid number of the mnts for each batch
        ...
        # The replace for rp.fill_diagonal_(0)
        B, N, _ = rp.shape
        indices = torch.arange(N)
        rp[:, indices, indices] = 0
        # rp.fill_diagonal_(0)
        # n_R = len(pairs) # changed for valid pairs number
        # change the torch.nan to 0
        ...
        # rp_sum = torch.sum(rp, axis=-1)
        # rp = rp.clamp(1e-10, None) # avoid the zero value
        for _ in range(n_rel): # iteration for relaxing
        ...
        # 以这个sorted_indices, 把lambda_t排序
        lambda_t_sorted = torch.gather(lambda_t, 1, sorted_indices)
        k_indices = n_pair.unsqueeze(1)  # 形状 [B, 1]
        mask = torch.arange(C).to(k_indices.device).expand(B, C) < k_indices
        lambda_t_sorted = lambda_t_sorted * mask
        # selected = k_largest_index_argpartition(efficiency, n_pair)[:, 0] # 以一个全局mask的形式存在
        # # pairs = pairs[selected] # do not need the pairs
        # score = torch.mean(lambda_t[selected])
        score = torch.sum(lambda_t_sorted, dim=-1) / n_pair
        return score#, lambda_t[selected]  #pairs,
    ...
    # pairs = pairs[(pairs[:,:,0] < n1) & (pairs[:,:,1] < n2)]  # 这里没有保证pairs的清洗，靠后续valid 或者有效值进行计算和判断
    pairs = pairs[:,:n1, :] # B, n1, 2
    # select the [B, n1] scores according to the pairs indexing
    scores = torch.gather(S, 2, pairs[:,:,1].unsqueeze(-1).repeat(1,1,1)).squeeze(-1)
    n1_batch = torch.sum(~torch.isnan(S[:,:,0]), dim=-1)
    n2_batch = torch.sum(~torch.isnan(S[:,0,:]), dim=-1)
    # n_pair = min_pair + torch.round(
    #     sigmoid(torch.tensor(float(min(n1, n2))), mu_p, tau_p) * (max_pair - min_pair)
    # ).int() # It needs to be a [B,] tensor
    min_number = torch.min(n1_batch, n2_batch)
    n_pair = min_pair + torch.round(sigmoid(min_number, mu_p, tau_p) * (max_pair - min_pair)).int()
    mnt1_order = torch.gather(mnt1, 1, pairs[:,:,0].unsqueeze(-1).repeat(1,1,3))
    mnt2_order = torch.gather(mnt2, 1, pairs[:,:,1].unsqueeze(-1).repeat(1,1,3)) # 以min_number的数量绝对最终匹配对的数量
    score = relax_labeling(mnt1_order, mnt2_order, scores, min_number, n_pair) # This is a relaxing batch
...
class Evaluator:
    def __init__(self, params, gpus, is_load=False, is_relax=False, Normalize=False, Binary=False) -> None:
        ...
        if is_load:
            ...
            self.dataset_pkl = f"./datasets/NISTvirt_eval.pkl" if self.NIST else f"./datasets/virt_eval.pkl"
            self.test_data = pickle.load(open(self.dataset_pkl, "rb"))[self.eval_dataset]
            # # each sample create one dataloader
            self.test_dataset = VirtDataset(
                self.prefix,
                f"./datasets/NISTvirt_eval.pkl" if self.NIST else f"./datasets/virt_eval.pkl", 
        ...
        save_folder = osp.join(self.prefix, self.eval_dataset, f'DMD_{self.ndim_feat}_virt') 
    ...
    def calculate_scores(self):
        ...
        with torch.no_grad():
            with tqdm(
                total=len(match_dataset), desc=f"Eval", ncols=min(160, int(os.popen("stty size").read().split()[1]))
            ) as pbar:
                for item in matchloader:
                    ...
                    if self.relax:
                        score = lsar_score_torchB(scores, search_mnt, gallery_mnt, min_pair=10, max_pair=100, mu_p=20, tau_p=0.4)
                    else:
                        score = lsa_score_torchB(scores, min_pair=10, max_pair=100, mu_p=20, tau_p=0.4)
                    ...
                    pbar.update(search_feat.shape[0])
    ...
    def eval_metric(self):
        if "FVC" not in self.eval_dataset:
            ...
            # for FVC evaluation, read the genuine pairs and imposter pairs
            # generate the score matrix
            df = pd.read_csv(self.score_file)
            score_mat = df.iloc[:,1:].values
            score_mat = np.where(np.isnan(score_mat), 0, score_mat) # invalid value process
            ...
            genuine_pairs = [x.strip().split(',') for x in genuine_pairs]
            genuine_pairs = np.array(genuine_pairs)
            imposter_pairs_file = osp.join(self.prefix, self.eval_dataset, 'imposter_pairs.txt')
            # load the name from the file
            with open(imposter_pairs_file, 'r') as f:
                imposter_pairs = f.readlines()
            imposter_pairs = [x.strip().split(',') for x in imposter_pairs]
            ...
            ixgrid = np.ix_(search_idx, gallery_idx)
            score_matrix[ixgrid] = score_mat
            # get the score for the genuine pairs
            search_idx = [search_imgs.index(x) for x in genuine_pairs[:,0]]
            gallery_idx = [gallery_imgs.index(x) for x in genuine_pairs[:,1]]
            genuine_score = score_matrix[search_idx, gallery_idx]
            # get the score for the imposter pairs
            search_idx = [search_imgs.index(x) for x in imposter_pairs[:,0]]
            gallery_idx = [gallery_imgs.index(x) for x in imposter_pairs[:,1]]
            imposter_score = score_matrix[search_idx, gallery_idx]
            total_score = np.concatenate([genuine_score, imposter_score])
            total_label = np.concatenate([np.ones_like(genuine_score), np.zeros_like(imposter_score)])
            # calculate the TAR
            TAR_flatten(total_score, total_label, dataname=self.eval_dataset)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--eval_dataset", "-d", type=str, default="NIST_SD27", required=True, help="The dataset for evaluation")
    parser.add_argument("--NIST", "-n", action="store_true", help="The dataset is from NIST")
    ...
    t = Evaluator(params, args.gpus, is_load=args.extract, is_relax=args.relax, Normalize=True, Binary=False) # pose is not necessary
