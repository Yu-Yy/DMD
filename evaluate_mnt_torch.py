...
def lsar_score_torchB(S, mnt1, mnt2, min_pair=4, max_pair=12, mu_p=20, tau_p=0.4):
    ...
    S2 = S
    max_n = max(n1, n2)
    # It cannot deal with the case that n1 > n2
    ...
    # consider the case that n1 may smaller than n2
    if n1 < n2:
        ...
        batch_set_pairs = batch_linear_assignment(new_S.transpose(1,2)) # transpose the matrix
        org_pair = torch.arange(new_S.shape[2])[None,...].repeat(B,1).to(new_S.device) # TODO: Maybe it is not used as to org_pair
        pairs = torch.stack((batch_set_pairs, org_pair), dim=-1) # It is batch and org 
        pairs = pairs[:,:n2, :]
    ...
    def __init__(self, params, gpus, is_load=False, is_relax=False, Normalize=False, Binary=False) -> None:
        ...
        if is_load:
            ...
            self.test_dataset = MntDataset(
                self.prefix,
                f"./datasets/NISTmnt_eval.pkl" if self.NIST else f"./datasets/mnt_eval.pkl", 
...
if __name__ == '__main__':
    ...
    t = Evaluator(params, args.gpus, is_load=args.extract, is_relax=True, Normalize=True, Binary=False) # pose is not necessary
